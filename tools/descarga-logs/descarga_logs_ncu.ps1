...
#   .\descarga_logs_ncu.ps1 -Topologia .\plantas\24019-san-jose.json   # la lista sale de la topologia de la toolbox
#   ncus.json: [{"ncu":"05","ip":"192.168.4.45"},{"ncu":"06","ip":"192.168.4.46"}]
#              (admite "usuario" y "pass" por NCU si alguna no sigue el esquema)
#   Credenciales: por defecto admin/NCU<nn>. Si una planta no lo sigue, se piden
#   UNA vez (enmascaradas) y se guardan cifradas (DPAPI) junto al script, por subred.
#
...
# ---- ncus.json desde la topologia de la TCU Toolbox ----
# El mismo plantas\<planta>.json que usa la toolbox ({plantas:[{nombre,ip,...}]}):
# el numero de NCU sale del nombre ("San Jose NCU7 GW2" -> 07) y la IP de la
# entrada; los dos gateways de una NCU comparten IP, asi que se dedupe por IP.
# Pura: se prueba sin red contra las topologias reales.
function Ncus-DeTopologia($j){
  $vistas=@{}; $r=@()
  foreach($p in @($j.plantas)){
    if(-not $p.ip){ continue }
    $m=[regex]::Match(("{0}" -f $p.nombre),'(?i)\bNCU\s*(\d+)')
    if(-not $m.Success){ continue }
    $ip=("{0}" -f $p.ip).Trim()
    if($vistas.ContainsKey($ip)){ continue }
    $vistas[$ip]=$true
    $r+=[pscustomobject]@{ ncu=("{0}" -f [int]$m.Groups[1].Value).PadLeft(2,"0"); ip=$ip }
  }
  return ,@($r | Sort-Object { [int]$_.ncu })
}
...
function UsuarioDe($n){
  if($n.PSObject.Properties["usuario"] -and $n.usuario){ return $n.usuario }
  if($script:CredPlanta){ return $script:CredPlanta.UserName }
  return $Usuario
}
function ClaveDe($n){
  if($n.PSObject.Properties["pass"] -and $n.pass){ return $n.pass }
  if($Password){ return $Password }
  if($script:CredPlanta){ return $script:CredPlanta.GetNetworkCredential().Password }
  return "NCU" + ("{0}" -f $n.ncu).PadLeft(2,"0")
}

# ---- credenciales de la planta, cifradas (DPAPI) ----
# Por defecto vale el esquema de Sunner (admin / NCU<nn>). Si una planta no lo
# sigue, se piden UNA vez con entrada enmascarada y se guardan con Export-Clixml:
# cifradas para tu usuario de Windows (copiadas a otro PC no valen), junto al
# script, nunca en el repo ni en claro en ncus.json. La tarea nocturna, que corre
# con el mismo usuario, las lee sola. Se guardan por planta, y la planta se
# reconoce por la subred de sus NCU (10.21.236 en San Jose): nada que configurar.
function SubredDe($ip){ return ((("{0}" -f $ip) -split ":")[0] -split "\.")[0..2] -join "." }
function PlantaDe($lista){
  if($Planta){ return $Planta }
  $p=@($lista)[0]; if($p -and $p.ip){ return (SubredDe $p.ip) }
  return "planta"
}
function RutaCred($planta){ Join-Path $PSScriptRoot ("credenciales.{0}.xml" -f (("{0}" -f $planta) -replace "[^\w\.-]","_")) }
function CredGuardada($planta){
  $r=RutaCred $planta
  if(Test-Path $r){ try{ return Import-Clixml $r }catch{ Log ("AVISO: credenciales guardadas ilegibles ({0}); se volveran a pedir" -f $r) } }
  return $null
}
function PedirYGuardarCred($planta){
  try{
    $c=Get-Credential -Message ("Usuario y contraseña del webserver de las NCU de {0} (el esquema admin/NCU<nn> no ha entrado)" -f $planta)
    if(-not $c){ return $null }
    $c | Export-Clixml (RutaCred $planta)
    Log ("credenciales de {0} guardadas cifradas en {1}" -f $planta,(RutaCred $planta))
    return $c
  } catch {
    Log ("FALLO login: el esquema admin/NCU<nn> no vale para {0} y no hay credenciales guardadas. Abre el menu (doble clic en el .bat) y lanza una descarga una vez para guardarlas." -f $planta)
    return $null
  }
}

$script:galletas=@{}
$script:CredPlanta=$null       # credenciales de la planta (DPAPI), si las hay
$script:PlantaActual="planta"
function Sesion($n,[bool]$forzar){
  $ip=$n.ip
  if($forzar -or (-not $galletas.ContainsKey($ip))){
    if($Cookie -and -not $forzar){ $galletas[$ip]=$Cookie }
    else{
      $tok=Login $ip (UsuarioDe $n) (ClaveDe $n)
      # el esquema no ha entrado y esta planta no tiene credenciales guardadas ni
      # forzadas: se piden UNA vez, se guardan cifradas y se reintenta el login
      if(-not $tok -and -not $script:CredPlanta -and -not $Password -and -not ($n.PSObject.Properties["pass"] -and $n.pass)){
        $c=PedirYGuardarCred $script:PlantaActual
        if($c){ $script:CredPlanta=$c; $tok=Login $ip (UsuarioDe $n) (ClaveDe $n) }
      }
      $galletas[$ip]=$tok
    }
  }
...
function Ejecuta($lista,$fechas){
  $script:galletas=@{}
  $script:PlantaActual=PlantaDe $lista
  $script:CredPlanta=CredGuardada $script:PlantaActual
  $bien=0; $mal=0
...
if($Ncus -or $Ip -or $Topologia){
  if($Topologia){ $lista=Ncus-DeTopologia (Get-Content $Topologia -Raw | ConvertFrom-Json); Log ("topologia {0}: {1} NCU" -f $Topologia,@($lista).Count) }
  elseif($Ncus){ $lista=Get-Content $Ncus -Raw | ConvertFrom-Json }
...
while($true){
  ...
  Write-Host "  8) Generar ncus.json desde una topologia de la toolbox (carpeta plantas\)"
  Write-Host "  0) Salir"
  $op=Read-Host "Opcion"
  switch($op){
    ...
    "8"{ $dirT=Join-Path $PSScriptRoot "plantas"
         $tops=@(Get-ChildItem $dirT -Filter *.json -ErrorAction SilentlyContinue | Where-Object { $_.Name -notlike "ambitos_*" } | Sort-Object Name)
         if(-not $tops){ Write-Host "No hay topologias en $dirT (en el zip de la toolbox van ahi)"; break }
         for($i=0;$i -lt $tops.Count;$i++){ Write-Host ("  {0}) {1}" -f ($i+1),$tops[$i].Name) }
         $e=Read-Host "Topologia (numero)"
         if($e -match '^\d+$' -and [int]$e -ge 1 -and [int]$e -le $tops.Count){
           $l8=Ncus-DeTopologia (Get-Content $tops[[int]$e-1].FullName -Raw | ConvertFrom-Json)
           (ConvertTo-Json -InputObject @($l8) -Depth 3) | Set-Content -Path $rutaNcus
           Write-Host ("ncus.json generado con {0} NCU desde {1}" -f @($l8).Count,$tops[[int]$e-1].Name)
           $l8 | ForEach-Object { Write-Host ("   NCU{0}  {1}" -f $_.ncu,$_.ip) }
         } else { Write-Host "Opcion no valida" } }
    "0"{ exit 0 }
