...
> ⚠️ **Un TCU cuelga de UN gateway.** Si sale en los dos, la toolbox lo sondea por los dos puertos y lo da por **caído** cuando no contesta por el que no es el suyo. Eso no da error en ningún sitio, y mirando la hoja no se ve: los dos rangos están en columnas distintas y hay que juntarlos para que salte. Le pasó a San José (NCU3, TCU 46) y se encontró de casualidad cruzando con el Excel de coordenadas; ahora `make_plantas.py` lo comprueba en **cada pasada y para todas las plantas**. Los **huecos** —un TCU que no cuelga de ningún gateway— se cuentan aparte y **no** como fallo: pueden ser una TCU retirada, que no renumera a las demás (Ayora NCU7 tiene el 14 así y El Burgo el 108), pero también fueron el síntoma de San José NCU9. Comprobado en `test_gateways_solape.py` (17 comprobaciones, 7 mutaciones).
